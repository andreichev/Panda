...
#    define ALIGNED_ALLOC(_allocator, _size, _align) Foundation::alloc(_allocator, _size, _align, __FILE__, __LINE__)
#    define ALIGNED_FREE(_allocator, _ptr, _align) Foundation::free(_allocator, _ptr, _align, __FILE__, __LINE__)
#    define ALIGNED_DELETE(_allocator, _ptr, _align) Foundation::deleteObject(_allocator, _ptr, _align, __FILE__, __LINE__)
#else
#    define ALLOC(allocator, size) Foundation::alloc(allocator, size, 0)
#    define FREE(allocator, ptr) Foundation::free(allocator, ptr, 0)
#    define DELETE(_allocator, _ptr) Foundation::deleteObject(_allocator, _ptr, 0, __FILE__, __LINE__)
...
#    define ALIGNED_DELETE(_allocator, _ptr, _align) Foundation::deleteObject(_allocator, _ptr, _align)
#endif

#define NEW(_allocator, _type) PLACEMENT_NEW(ALLOC(_allocator, sizeof(_type)), _type)
#define ALIGNED_NEW(_allocator, _type, _align) PLACEMENT_NEW(ALIGNED_ALLOC(_allocator, sizeof(_type), _align), _type)
...
struct AllocatorI {
    ...
    virtual void *realloc(void *ptr, size_t size, size_t _align, const char *file, uint32_t line) = 0;
};
...
void *alloc(AllocatorI *allocator, size_t size, size_t _align = 0, const char *file = nullptr, uint32_t line = 0);
